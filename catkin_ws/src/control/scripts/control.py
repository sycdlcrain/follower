...
def pure_pursuit(my_path):
	...
	lookahead_id = 4
	lookahead_id = np.minimum(lookahead_id,len(my_path)-1)
	local_goal = my_path[lookahead_id,:]
	print("lookahead", lookahead_id)
...
class control_node():
	def __init__(self):
		...
		try:
			...
			rospy.wait_for_message('/path_to_target', Marker, timeout=10.0)
	...
	def read_path(self, data):
		...
		# ~ print("---",p.x)
		self.my_path = np.asarray([[p.x,p.y,p.z] for p in data.points]) +0.0
		# ~ print(np.shape(self.my_path))
	...
	def get_commands(self):
		...
		self.steer = pure_pursuit(self.my_path)
		accel, self.vel = get_accel(self.vel, goal, target_speed, max_speed, dt)
